...
interface QrConfigDialogProps {
  ...
  onPlace:  (url: string, label: string) => void;
  onClose:  () => void;
}
...
interface QrOption {
  type:        QrType;
  label:       string;
  cardLabel:   string; // label shown on the printed QR card
  description: string;
  icon:        string;
  needsInput?: boolean;
  inputLabel?: string;
...
function buildICalContent(title: string, dateStr: string, location: string): string {
  ...
  return [
    ...
    `SUMMARY:${title}`, `DTSTART:${fmt(dt)}`, `DTEND:${fmt(end)}`,
...
export function QrConfigDialog({
  ...
  function getFinalUrl(): string {
    switch (selected) {
      case 'rsvp':      return rsvpUrl;
      case 'microsite': return siteUrl;
      case 'maps': {
      ...
      case 'vcard': {
        const vc = buildVCard(inputVal.trim() || organizerName, organizerPhone, organizerEmail);
      ...
      case 'custom': return inputVal.trim();
      default:       return '';
    }
  }
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 12, color: 'var(--ec-text-3)' }}>
            Choose what happens when guests scan — the card label updates automatically
          </p>
        </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      {active && (
                        <span style={{ fontSize: 10, color: 'var(--ec-text-3)', fontStyle: 'italic' }}>
                          card label: "{o.cardLabel}"
                        </span>
                      )}
                      {active && (
              ...
              <div style={{ marginTop: 6 }}>
                {organizerName ? (
                  ...
                  <p style={{ fontSize: 11, color: 'var(--ec-warning)', padding: '6px 10px', background: 'var(--ec-warning-bg)', border: '1px solid var(--ec-warning-border)', borderRadius: 6 }}>
                    ⚠ No organizer info set. Go to Event Settings to add your details.
                  </p>
                )}
              </div>
            )}
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <EcButton
            onClick={() => canPlace && onPlace(getFinalUrl(), opt.cardLabel)}
            disabled={!canPlace}
          >
            Add QR Code
          </EcButton>
