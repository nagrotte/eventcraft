...
export function CanvasShell({ eventId, eventTitle, eventDate, eventLocation, initialPrompt }: CanvasShellProps) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      aiBarRef.current?.triggerGenerate(initialPrompt);
    }, 800);
  }, [ready, initialPrompt]);
  ...
  useEffect(() => {
    ...
    checkDesign();
    return () => {
      canvas.off('object:added',   checkDesign);
      canvas.off('object:removed', checkDesign);
    };
  }, [ready, fabricRef]);
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        background: 'var(--ec-surface)', flexShrink: 0, gap: 12,
      }}>
        <span style={{ fontSize: 12, color: 'var(--ec-text-2)' }}>
          {eventTitle ?? 'Canvas Editor'}
        </span>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        {showImages && (
          <ImagePanel fabricRef={fabricRef} eventId={eventId} onClose={() => setShowImages(false)} />
        )}
        ...
        <div style={{
          ...
          backgroundSize: '20px 20px', overflow: 'auto', padding: 40,
        }}>
      ...
      {contextMenu && (
        <div ref={contextRef} style={{
          ...
          borderRadius: 'var(--ec-radius-md)', boxShadow: 'var(--ec-shadow-lg)',
          zIndex: 1000, minWidth: 140, padding: 4,
        }}>
          {[
            ...
            <button key={item.label} onClick={item.action} style={{
              ...
              color: (item as any).danger ? 'var(--ec-danger)' : 'var(--ec-text-1)',
